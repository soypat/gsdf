...
func (s SDF3) Union(others ...SDF3) SDF3 {
	...
	return wrap3(bld.Union(shaders...))
}
...
func (s SDF3) Diff(b SDF3) SDF3 {
	...
	if k > 0 {
		return wrap3(bld.SmoothDifference(float32(k), s.s, b.s))
	}
	return wrap3(bld.Difference(s.s, b.s))
}
...
func (s SDF3) Array(nx, ny, nz int, sx, sy, sz float64) SDF3 {
	return s.with(bld.Array(s.s, float32(sx), float32(sy), float32(sz), nx, ny, nz))
}
...
func (s SDF3) CircArray(count, circleDiv int) SDF3 {
	return s.with(bld.CircularArray(s.s, count, circleDiv))
}

// --- Output ---

// Save renders the shape to an STL file at filename.
// An optional [STLConfig] controls resolution and GPU usage; the zero value uses safe CPU defaults.
func (s SDF3) Save(filename string, cfg ...STLConfig) error {
