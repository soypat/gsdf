...
func Sphere(r float64) SDF3 { return wrap3(bld.NewSphere(float32(r))) }

// Box creates a box with the given dimensions and optional edge rounding radius.
func Box(x, y, z, round float64) SDF3 {
	return wrap3(bld.NewBox(float32(x), float32(y), float32(z), float32(round)))
}
...
func Cylinder(r, h, round float64) SDF3 {
	return wrap3(bld.NewCylinder(float32(r), float32(h), float32(round)))
}

// Torus creates a torus with the given major (ring) and minor (tube) radii.
func Torus(major, minor float64) SDF3 {
	return wrap3(bld.NewTorus(float32(major), float32(minor)))
}
...
func TriPrism(triHeight, extrudeLen float64) SDF3 {
	return wrap3(bld.NewTriangularPrism(float32(triHeight), float32(extrudeLen)))
}
...
func BoxFrame(x, y, z, edgeThickness float64) SDF3 {
	return wrap3(bld.NewBoxFrame(float32(x), float32(y), float32(z), float32(edgeThickness)))
}
