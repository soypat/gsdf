...
func (s SDF2) Union(others ...SDF2) SDF2 {
	...
	return wrap2("Union2D", bld.Union2D(shaders...))
}

// Diff subtracts b from s.
func (s SDF2) Diff(b SDF2) SDF2 { return wrap2("Diff2D", bld.Difference2D(s.s, b.s)) }

// Intersect returns the region inside both s and b.
func (s SDF2) Intersect(b SDF2) SDF2 { return wrap2("Intersect2D", bld.Intersection2D(s.s, b.s)) }

// Xor returns the region inside s or b but not both.
func (s SDF2) Xor(b SDF2) SDF2 { return wrap2("Xor2D", bld.Xor2D(s.s, b.s)) }

// --- Transforms (preserve pending k) ---

// Translate moves the shape by (x, y).
func (s SDF2) Translate(x, y float64) SDF2 {
	return s.with("Translate2D", bld.Translate2D(s.s, float32(x), float32(y)))
}

// Scale uniformly scales the shape by factor.
func (s SDF2) Scale(factor float64) SDF2 {
	return s.with("Scale2D", bld.Scale2D(s.s, float32(factor)))
}

// Rotate rotates the shape by radians around the origin.
func (s SDF2) Rotate(radians float64) SDF2 {
	return s.with("Rotate2D", bld.Rotate2D(s.s, float32(radians)))
}

// Mirror reflects the shape across the axes selected by x, y.
func (s SDF2) Mirror(x, y bool) SDF2 { return s.with("Mirror2D", bld.Symmetry2D(s.s, x, y)) }

// --- Shape modifiers (preserve pending k) ---

// Offset expands (positive delta) or contracts (negative delta) the shape's boundary.
func (s SDF2) Offset(delta float64) SDF2 {
	return s.with("Offset2D", bld.Offset2D(s.s, float32(delta)))
}

// Elongate stretches the shape along each axis by the given amounts.
func (s SDF2) Elongate(x, y float64) SDF2 {
	return s.with("Elongate2D", bld.Elongate2D(s.s, float32(x), float32(y)))
}
...
func (s SDF2) Array(nx, ny int, sx, sy float64) SDF2 {
	return s.with("Array2D", bld.Array2D(s.s, float32(sx), float32(sy), nx, ny))
}
...
func (s SDF2) CircArray(count, circleDiv int) SDF2 {
	return s.with("CircArray2D", bld.CircularArray2D(s.s, count, circleDiv))
}

// --- 2D → 3D conversions ---

// Extrude extrudes the 2D shape into a 3D solid of height h along Z.
func (s SDF2) Extrude(h float64) SDF3 { return wrap3("Extrude", bld.Extrude(s.s, float32(h))) }

// Revolve revolves the 2D shape around the Y axis, offset from the axis by offset units.
func (s SDF2) Revolve(offset float64) SDF3 {
	return wrap3("Revolve", bld.Revolve(s.s, float32(offset)))
}
